---
name: context7
description: "Live library documentation retrieval via Context7 API. Trigger when looking up current docs for npm packages, libraries, or frameworks instead of relying on potentially outdated training data. Use when agent needs current API docs."
globs: ["**"]
---
...
- **`global-mcp-lookup` skill** — Unified MCP phonebook (all servers, runtimes, tools)
- **`mcp-docker` skill** — Context7 is also available via `MCP_DOCKER_resolve-library-id` and `MCP_DOCKER_get-library-docs` when registered in the Docker MCP catalog
- **`mcp-registry.yaml`** — Machine-readable server index at `.claude/skills/global-mcp-lookup/references/mcp-registry.yaml`

**Architecture note:** Context7 has no Docker MCP server image. It is an API skill using direct HTTP calls. The `MCP_DOCKER_resolve-library-id` and `MCP_DOCKER_get-library-docs` tools provide an alternative MCP-gateway path when available.