...
**Tool namespace rule:** Codex may expose MCP tools as `mcp__allura-brain__memory_search`; OpenCode may expose the same operation as `allura-brain_memory_search`. Use whichever exact tool name is available in the current harness. Do not claim hydration happened unless one of these paths actually returned data.
