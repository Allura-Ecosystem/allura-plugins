...
User says: "Build this dashboard: https://figma.com/design/FIGMA_DASHBOARD_FILE_KEY_EXAMPLE/Dashboard?node-id=10-5"

**Actions:**

1. Parse URL to extract fileKey=`FIGMA_DASHBOARD_FILE_KEY_EXAMPLE` and nodeId=`10-5`
2. Run `get_metadata(fileKey="FIGMA_DASHBOARD_FILE_KEY_EXAMPLE", nodeId="10-5")` to understand the page structure
3. Identify main sections from metadata (header, sidebar, content area, cards) and their child node IDs
4. Run `get_design_context(fileKey="FIGMA_DASHBOARD_FILE_KEY_EXAMPLE", nodeId=":childNodeId")` for each major section
5. Run `get_screenshot(fileKey="FIGMA_DASHBOARD_FILE_KEY_EXAMPLE", nodeId="10-5")` for the full page
