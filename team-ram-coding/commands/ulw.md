...
3. **Classify task type** and assume appropriate Team RAM persona:
   ...
   - Discovery and debugging input → Scout (`.opencode/agent/subagents/core/scout.md`)
   - Database/schema → Woz (`.opencode/agent/subagents/code/woz.md`)
   - DevOps/infra → Brooks (`.opencode/agent/core/brooks.md`)
   - API/interface → Pike (`.opencode/agent/subagents/review/pike.md`)
   - Scope/acceptance → Jobs (`.opencode/agent/core/jobs.md`)
   - Research and search → Scout (`.opencode/agent/subagents/core/scout.md`)
