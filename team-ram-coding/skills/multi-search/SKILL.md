...
# Use local grep tool: pattern="moduleResolution" path=${CLAUDE_PROJECT_DIR}
```
...
# Use local grep tool: pattern="catch" include="*.ts" path=${CLAUDE_PROJECT_DIR}src/
```
...
# Use local grep tool: pattern="ECONNREFUSED" path=${CLAUDE_PROJECT_DIR}
...
Search our own codebase at `${CLAUDE_PROJECT_DIR}`. Instant, no rate limits, codebase-specific.
...
grep(pattern="SUPERSEDES", path="${CLAUDE_PROJECT_DIR}src/")

# Find all TypeScript files matching a pattern
grep(pattern="group_id", include="*.ts", path="${CLAUDE_PROJECT_DIR}src/")

# Find files by glob pattern
glob(pattern="**/*.test.ts", path="${CLAUDE_PROJECT_DIR}src/")
```
...
- **Scope:** Limited to `${CLAUDE_PROJECT_DIR}` codebase only.
